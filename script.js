...
// CFG sekme geçişleri (ayarlar sekmesinin içindeki)
const cfgTabs = document.querySelectorAll('.cfg-tab-menu .tab-btn');
const cfgContents = document.querySelectorAll('.cfg-tab');
cfgTabs.forEach(btn => {
  btn.addEventListener('click', () => {
    cfgTabs.forEach(b => b.classList.remove('active'));
    cfgContents.forEach(c => c.classList.remove('active'));
    btn.classList.add('active');
    document.getElementById(btn.dataset.tab).classList.add('active');
  });
});
...
const savedCfg = localStorage.getItem("myCS2Cfg");
if(savedCfg){
  parseCfg(savedCfg);
  renderTabs();
}
...
function parseCfg(content){
  ...
  cfgData = { game: [], video: [], mouse: [], map: [], keybind: [] };
  lines.forEach(line => {
    const trimmed = line.trim();
    if(!trimmed || trimmed.startsWith('//')) return;
    const parts = trimmed.split(' ');
    const key = parts[0], value = parts.slice(1).join(' ');
    if(key.startsWith('cl_') || key === "sensitivity") cfgData.game.push({key,value});
    else if(key.startsWith('mat_') || key.startsWith('r_')) cfgData.video.push({key,value});
    else if(key.startsWith('m_') || key.startsWith('mouse_')) cfgData.mouse.push({key,value});
    else if(key.startsWith('map_') || key.startsWith('cl_radar')) cfgData.map.push({key,value});
    else cfgData.keybind.push({key,value});
  });
}

// Tablo render
function renderTabs(){
  for(const cat in cfgData){
    const container = document.getElementById(cat);
    container.innerHTML = '';
    const table = document.createElement('table');
    const header = document.createElement('tr');
    header.innerHTML = `<th>Ayar</th><th>Değer</th>`;
    table.appendChild(header);

    cfgData[cat].forEach((item,i)=>{
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${item.key}</td><td><input type="text" value="${item.value}" data-cat="${cat}" data-index="${i}"></td>`;
      table.appendChild(tr);
    });
    container.appendChild(table);

    // Input değişikliklerini kaydet ve localStorage’a al
    container.querySelectorAll('input').forEach(input=>{
      input.addEventListener('input', e=>{
        const cat = e.target.dataset.cat;
        const idx = e.target.dataset.index;
        cfgData[cat][idx].value = e.target.value;

        let content = '';
        for(const c in cfgData){
          cfgData[c].forEach(i=> content += `${i.key} ${i.value}\n`);
        }
        localStorage.setItem("myCS2Cfg", content);
      });
    });
  }
}
...
// CFG indirme
const downloadBtn = document.getElementById('downloadCfg');
downloadBtn.addEventListener('click', ()=>{
  let content = '';
  for(const cat in cfgData){
    cfgData[cat].forEach(i=> content += `${i.key} ${i.value}\n`);
  }
  const blob = new Blob([content], {type:'text/plain'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'my_settings.cfg';
  a.click();
